...
def canonical(vertices, eye):
    """
    "Canonical" transformation of vertices
    """
    ...
    x_axis = torch.FloatTensor([1, 0, 0])
    y_axis = torch.FloatTensor([0, 1, 0])
    z_axis = torch.FloatTensor([0, 0, 1])
    ...
    if x_axis.ndimension() == 1:
        x_axis = x_axis[None, :].repeat(batch_size, 1)
    if y_axis.ndimension() == 1:
        y_axis = y_axis[None, :].repeat(batch_size, 1)
    if z_axis.ndimension() == 1:
        z_axis = z_axis[None, :].repeat(batch_size, 1)
    ...
    x_axis = F.normalize(x_axis, eps=1e-5)
    y_axis = F.normalize(y_axis, eps=1e-5)
    z_axis = F.normalize(z_axis, eps=1e-5)
